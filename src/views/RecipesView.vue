          ...
          >
            ...
            >
              ...
              <div class="bottom-right-button">
                <v-btn class="image-button" @click="toggleHeart(index)">
                  <img
                    :src="require(`../assets/images/red_heart.png`)"
                    :class="{ 'heart-icon': true, active: isLiked(recipe) }"
                    alt="Heart"
                  />
                </v-btn>
                <div v-if="isLiked(recipe)" class="liked-users">
                  Liked by {{ getLikedUserCount(recipe) }} user{{
                    getLikedUserCount(recipe) > 1 ? "s" : ""
                  }}
                  <template v-for="user in recipe.likedUsers">
                    <span
                      v-if="user === $store.state.user.email"
                      class="liked-user"
                      >{{ user }}</span
                    >
                  </template>
  ...
  methods: {
    ...
    toggleHeart(index) {
      ...
      if (recipe.likedUsers.includes(userEmail)) {
        axios
          .post(`http://localhost:3000/recipes/${recipe.id}/unlike`, {
            userEmail: userEmail,
          })
          .then(() => {
            // Recipe unliked successfully, update the frontend
            const userIndex = recipe.likedUsers.findIndex(
        ...
        axios
          .post(`http://localhost:3000/recipes/${recipe.id}/like`, {
            userEmail: userEmail,
          })
          .then(() => {
            // Recipe liked successfully, update the frontend
    ...
    isLiked(recipe) {
      const userEmail = this.$store.state.user.email; // Replace with the actual location of the user's email in your state management system
      return recipe.likedUsers.includes(userEmail);
    },
    getLikedUserCount(recipe) {
      const userEmail = this.$store.state.user.email; // Replace with the actual location of the user's email in your state management system
      return recipe.likedUsers.filter((user) => user === userEmail).length;
    },
  },
};
</script>
